...
def analyze_audio(url, loop=None, status_msg=None):
    """
    Запускает конвейер: скачивание -> анализ через Gemini API (cloud audio) -> Edge-TTS.
    """
    ...
    try:
        ...
        download_success = core.download_media(
            ...
            audio_only=True,
            progress_callback=lambda p: update_status(f"📥 [2/4] Скачиваю аудио: {p}\n«{clean_title}»...")
        )
        if not download_success:
            return None, None, None

        # ЭТАП 2 и 3: Загрузка в облако и анализ ИИ
        # Сохраняем модель gemini-3-flash, как в оригинале
        summary_text = core.analyze_cloud_audio(
            client=core.client,
            audio_path=audio_path,
            model="gemini-3-flash",
            progress_callback=lambda status: update_status(status.replace("📥 [2/4]", "").replace("🤖 [4/4]", ""))
        )
        ...
        # ЭТАП 4: Озвучка выжимки через Edge-TTS
...
def analyze_text_article(url, loop=None, status_msg=None):
    """
    Запускает конвейер для статей: скачивание текста -> анализ через Gemini API -> Edge-TTS.
    """
    def update_status(text):
        print(text)
        if loop and status_msg:
    ...
    try:
        ...
        update_status("🎙️ [3/4] Создаю аудио-выжимку...")
        tts_success = core.run_edge_tts(
            text=summary_text,
            output_path=tts_audio_path,
            voice="ru-RU-DmitryNeural"
